...
public abstract class Matrix {
    ...
    public Vector getRow(int row){
        Vector rowVector = Vector.FACTORY.makeZero(getNumCols());
        for(int i=0; i < getNumCols(); i++)
            rowVector.set(i, get(row, i));
        return rowVector;
    }

    /**
     * @param col: column to return
     * @return a copy of a given column of the matrix
     * @throws ArrayIndexOutOfBoundsException if col is negative or larger than getNumColumns()
     */
    public Vector getColumn(int col){
        Vector colVector = Vector.FACTORY.makeZero(getNumRows());
        for(int i=0; i < getNumRows(); i++)
            colVector.set(i, get(i, col));
        return colVector;
    }

    /**
     * @param start: start column
     * @param end: end column (not included)
    ...
    public Matrix transpose(){
        Matrix transpose = FACTORY.makeZero(getNumCols(), getNumRows());
        for(int i=0; i < getNumRows(); i++)
            for(int j=0; j < getNumCols(); j++)
